...
function getUpdatedFilmDoc(existingFilmDoc, filmDetailsObject) {
    const updatedFilmDoc = updateAverageRatingString(existingFilmDoc, filmDetailsObject.averageRatingString);
    return updatedFilmDoc;
}
...
async function saveScrapedFilmDetailsObject(filmDetailsObject) {
    ...
    const savedFilmDoc = existingFilmDoc != null
...
async function scrapePopularFilmsList() {
    await processFilmsInList('https://letterboxd.com/bucksalypse/list/letterboxd-500-most-watched-movies-of-all/', saveScrapedFilmDetailsObject);
}
